...
class DatasetDownloader:
    ...
    def __init__(self, url: str, project_id: int, api_key: str):
        ...
        self.LsClient = Client(url=url, api_key=api_key)
    ...
    def _verify_label_studio(self):
        try:
            ...
            if health.get("status") != "UP":
                raise RuntimeError(f"Label Studio is unhealthy: {health}")

        except Exception as e:
            raise ConnectionError(f"Unable to connect to Label Studio: {e}") from e
